...
import com.basrikahveci.p2p.peer.network.message.file.FileMessage;
...
public class Peer {
    ...
    public void sendFile(String peerName, String filePath) {
    if (isShutdown()) {
        LOGGER.warn("Cannot send file. Peer is not running.");
        return;
    }

    Connection connection = connectionService.getConnection(peerName);
    if (connection == null) {
        LOGGER.warn("No connection found for peer: {}", peerName);
        return;
    }
    ...
    try {
        ...
        if (!file.exists() || !file.isFile()) {
            LOGGER.warn("Archivo no válido: {}", filePath);
            return;
        }

        FileMessage fileMessage = FileMessage.fromFile(file);
        connection.send(fileMessage);
        LOGGER.info("Archivo enviado: {} a {}", file.getName(), peerName);
    } catch (IOException e) {
        LOGGER.error("Error al leer o enviar archivo", e);
    }
}
